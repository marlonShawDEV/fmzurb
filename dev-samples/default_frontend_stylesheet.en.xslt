...
<!-- Flag to signal if current mode is embeddeded or not.<strong></strong>
     '1' - yes, '0' - No -->
...
<xsl:variable name="logo_url">http://www.freddiemac.com/images/search/search_logo.gif</xsl:variable>
...
<xsl:variable name="global_font"></xsl:variable>
<xsl:variable name="global_font_size">1rem</xsl:variable>
<xsl:variable name="global_bg_color">#ffffff</xsl:variable>
<xsl:variable name="global_text_color">#464645</xsl:variable>
<xsl:variable name="global_link_color">#00a6e2</xsl:variable>
<xsl:variable name="global_vlink_color">#00a6e2</xsl:variable>
<xsl:variable name="global_alink_color">#00a6e2</xsl:variable>
...
<xsl:variable name="search_box_size">50</xsl:variable>
...
<xsl:variable name="sep_bar_res_text">Search Results</xsl:variable>
<xsl:variable name="sep_bar_swr_text">Search Within Results</xsl:variable>
...
<xsl:variable name="my_nav_size">13.2px</xsl:variable>
...
<xsl:variable name="synonyms_text_color">#333333</xsl:variable>

<!-- *** keymatch suggestions *** -->
<xsl:variable name="show_keymatch">1</xsl:variable>
<xsl:variable name="keymatch_text">Editor's Pick</xsl:variable>
<xsl:variable name="keymatch_text_color">#676767</xsl:variable>
<xsl:variable name="keymatch_bg_color">#fff</xsl:variable>
...
<xsl:variable name="res_title_size">18px</xsl:variable>
<xsl:variable name="show_res_snippet">1</xsl:variable>
<xsl:variable name="res_snippet_size">16px</xsl:variable>
...
<xsl:variable name="res_url_color">inherit</xsl:variable>
<xsl:variable name="res_url_size">inherit</xsl:variable>
...
<xsl:template match="Record">
 ...
 Logo template (can be customized)
  ...
  <xsl:template name="logo">
    <div>
      <a href="{$home_url}">
        <img src="{$logo_url}"
      alt="Go to site search home" border="0" />
      </a>
    </div>
  </xsl:template>
 ...
 Search result page header (can be customized): logo and search box
  ...
  <xsl:template name="result_page_header">
    ...
    <table class="searchBoxTop" border="0" cellpadding="0" cellspacing="0">
      <xsl:if test="$show_top_search_box != '0'">
        <tr>
          <td class="searchBoxTop" valign="top">
 ...
 Search result page header (can be customized): logo and search box
     ********************************************************************** -->
  <xsl:template name="fm_result_page_header">
      <xsl:if test="$show_top_search_box != '0'">
        <div>
          <xsl:call-template name="fm_search_box">
            <xsl:with-param name="type" select="'std_top'"/>
          </xsl:call-template>
        </div>
      </xsl:if>
 ...
 Home search page header (can be customized): logo and search box
     ********************************************************************** -->
  <xsl:template name="fm_swr_page_header">
      <xsl:if test="$show_top_search_box != '0'">
        <div>
          <xsl:call-template name="fm_search_box">
            <xsl:with-param name="type" select="'swr'"/>
 ...
 Home search page header (can be customized): logo and search box
     ********************************************************************** -->
  <xsl:template name="fm_home_page_header">
      <xsl:if test="$show_top_search_box != '0'">
        <div>
          <xsl:call-template name="fm_search_box">
...
<xsl:variable name="sep_bar_bg_color">
  <xsl:choose>
    ...
    <xsl:when test="$choose_sep_bar = 'blue'">#3366cc</xsl:when>
    <xsl:otherwise><xsl:value-of select="$keymatch_bg_color"/></xsl:otherwise>
  </xsl:choose>
</xsl:variable>
  ...
  <tr>
    <td>
      <table border="1" width="100%" cellpadding="10" cellspacing="0"
        ...
        <tr>
          <td>
            <font face="{$global_font}" color="{$global_text_color}">
 ...
 "Search Within Results" search input page (can be customized)
  ...
  <xsl:template name="swr_search">
    ...
    <html>
      ...
      <body dir="ltr">
            <xsl:call-template name="fm_header"/> 
            <xsl:call-template name="fm_top_sep_bar">
              <xsl:with-param name="text" select="$sep_bar_swr_text"/>
            </xsl:call-template>                 
            <xsl:call-template name="personalization"/>
            <xsl:call-template name="analytics"/>  
            <xsl:call-template name="fm_swr_page_header"/>
            <div class="content-band">
              <div class="row column">
              </div>
            </div>            
            <!-- <xsl:call-template name="my_page_footer"/>  -->
            <xsl:call-template name="fm_footer"/>
        <xsl:call-template name="fm_jsfooter"/>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:call-template name="doc_type"/>
    <html>
      ...
      <body dir="ltr">
            ...
            <xsl:call-template name="fm_home_page_header"/>     
            <div class="content-band">
              <div class="row column">
              </div>
            </div>
...
body,.d,.p{background-color:<xsl:value-of select="$global_bg_color"/>} -->
.s { padding: 0; line-height: 1.3;}
b { font-weight: 500; }
.l { font-size: 1.125rem;  font-weight: 500;}
.l b { font-weight: 900; }
.g, .no-g { margin-bottom: 0; font-weight: 400; border-width: 0; font-size: inherit; }
.s { font-weight: 300; }
table .s { overflow: hidden;}
blockquote.g { margin-left: 2rem; }
p.g + table, p.no-g + table { margin-bottom: 1.5rem; }
.n { font-weight: 500; font-size: 1.125rem; margin-top: 4rem; text-align: center; }
.n a { font-weight: 300; }
.n .b a { font-weight: 500; }
...
div#clustering h3 {font-size: 100%; font-weight: 700; margin: 0; padding: 0;}
...
div#clustering h3 {font-size: 100%; font-weight: 700; margin: 0 0 0.6em 0; padding: 0;}
...
<xsl:comment>
   ...
   */
@media screen and (max-width: 33.62499em) {
  div#dyn_nav { display: none; }
}
@media screen and (min-width: 33.625em) {
  div#main_res {
    <xsl:choose>
...
@media screen and (min-width: 65em) {
  ...
  .dn-img {
    background:transparent url("data:image/svg+xml;charset=utf8,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20xmlns:xlink=%22http://www.w3.org/1999/xlink%22%20width=%2216%22%20height=%2216%22%20viewBox=%220%200%20170%20170%22%3E%3Cpath%20fill=%22%2300a6e2%22%20d=%22M83.7%200a83.7%2083.7%200%201%200%2083.7%2083.7A83.7%2083.7%200%200%200%2083.7%200zm42.67%20127.06a6.13%206.13%200%200%201-8.67-.07l-34-34.55L49.69%20127a6.13%206.13%200%201%201-8.74-8.6L75.1%2083.7%2041%2049a6.13%206.13%200%201%201%208.74-8.6L83.7%2075l34-34.55a6.13%206.13%200%201%201%208.74%208.6L92.29%2083.7l34.14%2034.69a6.13%206.13%200%200%201-.06%208.67z%22%20/%3E%3C/svg%3E") no-repeat 0 0;
    width:1em;
    height:1em;
    margin-right: 5px;
...
<xsl:comment>
  ...
  body.previews-enabled span.toggle-preview {
    ...
    height: 18px;
    opacity: 0.5;
    background-image: url("data:image/svg+xml;charset=utf8,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20xmlns:xlink=%22http://www.w3.org/1999/xlink%22%20width=%2218%22%20height=%2218%22%20viewBox=%220%200%20512%20460%22%3E%3Cpath%20fill=%22%2300a6e2%22%20d=%22M445%20386.7l-84.8-85.9c13.8-24.1%2021-50.9%2021-77.9%200-87.6-71.2-158.9-158.6-158.9C135.2%2064%2064%20135.3%2064%20222.9c0%2087.6%2071.2%20158.9%20158.6%20158.9%2027.9%200%2055.5-7.7%2080.1-22.4l84.4%2085.6c1.9%201.9%204.6%203.1%207.3%203.1%202.7%200%205.4-1.1%207.3-3.1l43.3-43.8C449%20397.1%20449%20390.7%20445%20386.7zM222.6%20125.9c53.4%200%2096.8%2043.5%2096.8%2097%200%2053.5-43.4%2097-96.8%2097%20-53.4%200-96.8-43.5-96.8-97C125.8%20169.4%20169.2%20125.9%20222.6%20125.9z%22%20/%3E%3C/svg%3E");
    background-position: center center;
    background-repeat: no-repeat;
    background-origin: content-box;
	  background-size: 18px 18px;
  }
  body.previews-enabled div.result-item-hover span.toggle-preview {
    color: #0000cc;
    opacity: 0.7;
  }
  body.previews-enabled span.toggle-preview {
...
<xsl:call-template name="fm_styles"/>
</xsl:template>
...
<xsl:template match="GSP">
  <xsl:choose>
    ...
    <xsl:when test="CUSTOM/ADVANCED">
      <xsl:call-template name="fm_advanced_search"/>
    </xsl:when>
    <xsl:when test="ERROR">
...
<xsl:template name="js_escape">
 ...
 Advanced search page (do not customize)
  ...
  <xsl:template name="advanced_search">
                            ...
                            <tr>
                             ...
                             size=&quot;25&quot; value=&quot;</xsl:text>
                              ...
                              <td>
                                  ...
                                  <input type="submit" name="btnG" class="button secondary"
                    ...
                    cellspacing="0" border="0">
                      <tr>
                        ...
                        <td width="40%" nowrap="nowrap">
                            ...
                            return results from the category
                            ...
                            <select name="site">
                              <xsl:choose>
                                <xsl:when test="/GSP/PARAM[@name='site']/@value = 'default_collection'">
                                  <option value="default_collection" selected="selected">All</option>
                                </xsl:when>
                                <xsl:otherwise>
                                  <option value="default_collection">
                                    All
                                  </option>
                                </xsl:otherwise>
                              </xsl:choose>                              
                          ...
                          border="0">
                            ...
                            <tr>
                              <td valign="top" nowrap="nowrap">
                                <div class="example">e.g. freddiemac.com, loanprospector.com</div>
                              </td>
                            </tr>
                          </table>
                        </td>
              ...
              <table cellpadding="2" cellspacing="0" border="0">
                <form method="get" action="search" name="h">
                  <tr bgcolor="{$global_bg_color}">
                    ...
                    <td>Find pages that link to the page</td>
                    <td>
                      <xsl:choose>
					 ...
					 <td>
                      <input type="submit" name="btnG" class="button secondary" value="{$search_button_text}" />
                    </td>
                  </tr>
                </form>
              </table>
   ...
   <!-- **********************************************************************
 Advanced search page (custom version)
     ********************************************************************** -->
<xsl:template name="fm_advanced_search">
  <xsl:variable name="html_escaped_as_q">
    <xsl:call-template name="escape_quot">
      <xsl:with-param name="string">
        <xsl:choose>
  ...
  <html>
    ...
    <body onload="setFocus()"  dir="ltr">
      ...
      <xsl:call-template name="analytics"/>
      <!--====Advanced Search Header======-->
      <xsl:if test="$choose_adv_search_page_header = 'provided' or $choose_adv_search_page_header = 'both'">
        <xsl:call-template name="advanced_search_header"/>
      </xsl:if> 
      <div class="content-band">
        <div class="row column">        
          <!--====Carry over Search Parameters======-->
          <form method="get" action="search" name="f" class="form">
            <xsl:if test="PARAM[@name='client']">
              <input type="hidden" name="client"
                value="{PARAM[@name='client']/@value}" />
            </xsl:if>
            ...
            <p class="field clearfix">
                <xsl:text disable-output-escaping="yes">
                  &lt;input type=&quot;text&quot; id=&quot;field1&quot;name=&quot;as_q&quot; class=&quot;text&quot;size=&quot;25&quot; value=&quot;</xsl:text>
                <xsl:value-of disable-output-escaping="yes" select="$html_escaped_as_q"/>
                <xsl:text disable-output-escaping="yes">&quot;&gt;</xsl:text> 
              <label for="field1">with <b>all</b> of the words</label>
            </p>   
            <p class="field clearfix">                
                <xsl:text disable-output-escaping="yes">
                  &lt;input type=&quot;text&quot; id=&quot;field2&quot;name=&quot;as_epq&quot; class=&quot;text&quot;size=&quot;25&quot; value=&quot;</xsl:text>
                <xsl:value-of disable-output-escaping="yes" select="$html_escaped_as_epq"/>
                <xsl:text disable-output-escaping="yes">&quot;&gt;</xsl:text>
              <label for="field2">with the <b>exact phrase</b></label>
            </p>   
            <p class="field clearfix">              
                <xsl:text disable-output-escaping="yes">
                  &lt;input type=&quot;text&quot; id=&quot;field3&quot;name=&quot;as_oq&quot; class=&quot;text&quot;size=&quot;25&quot; value=&quot;</xsl:text>
                <xsl:value-of disable-output-escaping="yes" select="$html_escaped_as_oq"/>
                <xsl:text disable-output-escaping="yes">&quot;&gt;</xsl:text>
              <label for="field3">with <b>at least one</b> of the words</label>
            </p>   
            <p class="field clearfix">                
                <xsl:text disable-output-escaping="yes">
                  &lt;input type=&quot;text&quot; id=&quot;field4&quot;name=&quot;as_eq&quot; class=&quot;text&quot;size=&quot;25&quot; value=&quot;</xsl:text>
                <xsl:value-of disable-output-escaping="yes" select="$html_escaped_as_eq"/>
                <xsl:text disable-output-escaping="yes">&quot;&gt;</xsl:text>                
              <label for="field4"><b>without</b> the words</label>
            </p>   
            
            <p>
              <label for="num_options" class="weight-medium">Number of results to display</label>                
              <span id="num_options">  
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='num') and (@value!='10')]">
                    <input type="radio" value="10" name="num" id="num_10"/><label for="num_10">10</label>
                  </xsl:when>
                  <xsl:otherwise>
                    <input type="radio" value="10" name="num" id="num_10" checked="checked"/><label for="num_10">10</label>
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='num') and (@value='20')]">
                    <input type="radio" value="20" name="num" id="num_20" checked="checked"/><label for="num_20">20</label>
                  </xsl:when>
                  <xsl:otherwise>                                        
                    <input type="radio" value="20" name="num" id="num_20"/><label for="num_20">20</label>
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='num') and (@value='30')]">
                    <input type="radio" value="30" name="num" id="num_30" checked="checked"/><label for="num_30">30</label>
                  </xsl:when>
                  <xsl:otherwise>                                        
                    <input type="radio" value="30" name="num" id="num_30"/><label for="num_30">30</label>
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='num') and (@value='50')]">
                    <input type="radio" value="50" name="num" id="num_50" checked="checked"/><label for="num_50">50</label>
                  </xsl:when>
                  <xsl:otherwise>
                    <input type="radio" value="50" name="num" id="num_50"/><label for="num_50">50</label>
                  </xsl:otherwise>
                </xsl:choose>
              </span>
            </p>
            <p>
              <label  for="as_ft" class="weight-medium">Limit by file format</label>
              <select class="small-field" name="as_ft">
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='as_ft') and (@value='i')]">
                    <option value="i" selected="selected">Only include</option>
                  </xsl:when>
                  <xsl:otherwise>
                    <option value="i">Only include</option>
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='as_ft') and (@value='e')]">
                    <option value="e" selected="selected">Don't include</option>
                  </xsl:when>
                  <xsl:otherwise>
                    <option value="e">Don't include</option>
                  </xsl:otherwise>
                </xsl:choose>
              </select>
              <select class="small-field" name="as_filetype">
                <xsl:choose>
                  <xsl:when test="PARAM[(@name='as_filetype') and (@value!='')]">
            ...
            <p>
              <label for="occt_options" class="weight-medium">Return results where my terms occur</label>
              <span id="occt_options">                           
                <xsl:choose>
                    <xsl:when test="PARAM[(@name='as_occt') and (@value!='any')]">
                      <input name="as_occt" type="radio" value="any" id="occt_0"/><label for="occt_0">anywhere</label>                           
                    </xsl:when>
                    <xsl:otherwise>
                      <input name="as_occt" type="radio" value="any" id="occt_0" checked="checked"/><label for="occt_0">anywhere</label>
                    </xsl:otherwise>
                  </xsl:choose>
                  <xsl:choose>
                    <xsl:when test="PARAM[(@name='as_occt') and (@value='title')]">
                      <input name="as_occt" type="radio" value="title" id="occt_1" checked="checked"/><label for="occt_1">in the title</label> 
                    </xsl:when>
                    <xsl:otherwise>
                      <input name="as_occt" type="radio" value="title" id="occt_1"/><label for="occt_1">in the title</label>                              
                    </xsl:otherwise>
                  </xsl:choose>
                  <xsl:choose>
                    <xsl:when test="PARAM[(@name='as_occt') and (@value='url')]">
                      <input name="as_occt" type="radio" value="url" id="occt_2" checked="checked"/><label for="occt_2">in the URL</label>
                    </xsl:when>
                    <xsl:otherwise>
                      <input name="as_occt" type="radio" value="url" id="occt_2"/><label for="occt_2">in the URL</label>
                    </xsl:otherwise>
                  </xsl:choose>        
              </span>
            </p>
            ...
            <p class="hide">
              <label for="as_dt" class="weight-medium">Limit by site or domain</label>  
                <select class="small-field"  name="as_dt">
                  <xsl:choose>
                    <xsl:when test="PARAM[(@name='as_dt') and (@value='i')]">
                      <option value="i" selected="selected">Only include</option>
                    </xsl:when>
                ...
                <xsl:choose>
                  <xsl:when test="PARAM[@name='as_sitesearch']">
                    <input class="small-field" type="text" size="25" value="{PARAM[@name='as_sitesearch']/@value}" name="as_sitesearch" />
                  </xsl:when>
                  <xsl:otherwise>
                    <input class="small-field" type="text" size="25" value="" name="as_sitesearch" />
                  </xsl:otherwise>
                </xsl:choose>
              <span> e.g. freddiemac.com, loanprospector.com</span>
            </p> 
            <p>                       
              <button type="submit" class="button secondary" name="btnG"><xsl:value-of select="$search_button_text"/></button> 
            </p> 
          </form>
          ...
          <form method="get" action="search" name="h" class="form"> 
            <xsl:if test="PARAM[@name='client']">
            ...
            <xsl:if test="PARAM[@name='output']">
              <input type="hidden" name="output" value="{PARAM[@name='output']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='proxystylesheet']">
              <input type="hidden" name="proxystylesheet" value="{PARAM[@name='proxystylesheet']/@value}" />
            </xsl:if>
            <!--<xsl:if test="PARAM[@name='site']">
              <input type="hidden" name="site"  value="{PARAM[@name='site']/@value}" />
            </xsl:if>-->
            <xsl:if test="PARAM[@name='ie']">
              <input type="hidden" name="ie" value="{PARAM[@name='ie']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='oe']">
              <input type="hidden" name="oe" value="{PARAM[@name='oe']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='hl']">
              <input type="hidden" name="hl" value="{PARAM[@name='hl']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='getfields']">
              <input type="hidden" name="getfields" value="{PARAM[@name='getfields']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='requiredfields']">
              <input type="hidden" name="requiredfields" value="{PARAM[@name='requiredfields']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='partialfields']">
              <input type="hidden" name="partialfields" value="{PARAM[@name='partialfields']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='exclude_apps']">
              <input type="hidden" name="exclude_apps" value="{PARAM[@name='exclude_apps']/@value}" />
            </xsl:if>
            <xsl:if test="PARAM[@name='only_apps']">
              <input type="hidden" name="only_apps" value="{PARAM[@name='only_apps']/@value}" />
            </xsl:if> 
            <p>
              <label  for="as_lq" class="weight-medium">Find pages that link to the page</label>
                <xsl:choose>
                  <xsl:when test="PARAM[@name='as_lq']">
                    <input type="text" size="30" id="as_lq" 
                     value="{PARAM[@name='as_lq']/@value}"
                             name="as_lq" />
                  </xsl:when>
                  <xsl:otherwise>
                    <input type="text" size="30" value="" name="as_lq" id="as_lq"  />
                  </xsl:otherwise>
                </xsl:choose>      
            </p> 
            <p>
        ...
        <xsl:if test="RES/R">
          ...
          <div style="display: none;" id="bottom-search-box-html">
            <xsl:if test="$show_bottom_search_box != '0' and RES">
              <xsl:call-template name="fm_bottom_search_box"/>
            </xsl:if>
          </div>
        </xsl:if>
      </div>
...
<xsl:template name="search_results_body">
  ...
  <!-- Send across form parameters that's used in the GSA search form, if we are
    ...
    <xsl:call-template name="fm_header"/>   
    <!-- *** Customer's own result page header *** -->
    <xsl:if test="$choose_result_page_header = 'mine' or $choose_result_page_header = 'both'">
      <xsl:call-template name="my_page_header"/>
    </xsl:if>
    <xsl:call-template name="fm_top_sep_bar">
      <xsl:with-param name="text" select="$sep_bar_res_text"/>
    </xsl:call-template>
    
    <!-- *** Result page header *** -->
    <xsl:if test="$choose_result_page_header = 'provided' or $choose_result_page_header = 'both'">
      <xsl:call-template name="fm_result_page_header" />
    </xsl:if>

    <!-- *** Top separation bar *** -->
     <xsl:if test="Q != ''">		
        <xsl:call-template name="fm_bottom_sep_bar">
          <xsl:with-param name="show_info" select="1"/>
        </xsl:call-template>
      </xsl:if>
      <div class="content-band">
        <div class="row column">
...
<select name="site">
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='annarbor')]">
   <option value="annarbor" selected="selected">annarbor</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='bizContacts')]">
   <option value="bizContacts" selected="selected">bizContacts</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="bizContacts">bizContacts</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='bizLocations')]">
   <option value="bizLocations" selected="selected">bizLocations</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="bizLocations">bizLocations</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='clients')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='demo_translation')]">
   <option value="demo_translation" selected="selected">demo_translation</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="demo_translation">demo_translation</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='employees')]">
   <option value="employees" selected="selected">employees</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='google')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='google_de')]">
   <option value="google_de" selected="selected">google_de</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="google_de">google_de</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='google_es')]">
   <option value="google_es" selected="selected">google_es</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="google_es">google_es</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='google_ja')]">
   <option value="google_ja" selected="selected">google_ja</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="google_ja">google_ja</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='onixapps')]">
   <option value="onixapps" selected="selected">onixapps</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="onixapps">onixapps</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='onixappsBlueprints')]">
   <option value="onixappsBlueprints" selected="selected">onixappsBlueprints</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="onixappsBlueprints">onixappsBlueprints</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='onixappsEmployees')]">
   <option value="onixappsEmployees" selected="selected">onixappsEmployees</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="onixappsEmployees">onixappsEmployees</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='pdf')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='sharepoint2010')]">
   <option value="sharepoint2010" selected="selected">sharepoint2010</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="sharepoint2010">sharepoint2010</option>
  </xsl:otherwise>
 </xsl:choose>
</select>
          ...
          <tr><td>
                <table cellpadding="4" cellspacing="0">
                  <tr><td>
                    ...
                    There were about <b><xsl:value-of select="RES/M"/></b> results for <b><xsl:value-of select="$cleared_q"/></b>.
                      <br/>
                    </xsl:if>
        ...
        <tr>
          ...
          <font size="-1">
            <xsl:choose>
              ...
              <xsl:when test="$show_suggest = '1' and (($type = 'home') or ($type = 'std_top'))">
                ...
                <tr><td>
                <input type="text" name="q" class="q" size="{$search_box_size}" maxlength="256" value="{$cleared_q}"/>
                </td></tr>
                </table>
              </xsl:when>
              <xsl:otherwise>
          ...
          <td valign="middle">
            <xsl:call-template name="nbsp"/>
              ...
              <xsl:otherwise>
                <input type="submit" class="btnSearch" name="btnG" value="{$search_button_text}"/>
              </xsl:otherwise>
              </xsl:choose>
          </td>
		    <td nowrap="1"><a href="{$search_url}&amp;q={$cleared_q}" type="submit"  class="btnSearch" name="fmSearch" style="background:#888; margin: 0 1em; line-height: 16px;" ><xsl:value-of select="$search_button_text"/></a></td>
                  <td nowrap="1">
                        <font size="-2">
...
<xsl:template name="fm_search_box">
  <xsl:param name="type"/>
  <div class="background-concrete content-band search-band">
    <div class="row column">  
      <div class="search-field-container clearfix">
        <div class="search-form-wrapper"> 
        <xsl:if test="$is_embedded_mode != '1'">
          <xsl:choose>
            <xsl:when test="$show_suggest = '1' and (($type = 'home') or ($type = 'std_top'))">
              <xsl:text disable-output-escaping="yes">&lt;form id="suggestion_form" name="gs" method="GET" action="search" onsubmit="return (this.q.value == '') ? false : true;" class="form"&gt;</xsl:text>
            </xsl:when>
            <xsl:otherwise>
            <xsl:text disable-output-escaping="yes">&lt;form name="gs" method="GET" action="search" onsubmit="return (this.q.value == '') ? false : true;" class="form"&gt;</xsl:text>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:if>
        <xsl:if test="($type = 'swr')">
          <div class="block">                    
            <xsl:variable name="est_result" select="/GSP/RES/M" />
            <xsl:if test="($est_result != '') and ($est_result > 0)">
              <p class="flex-up">There were about <b><xsl:value-of select="RES/M"/></b> results for <b><xsl:value-of select="$cleared_q"/></b>.</p>
            </xsl:if>
          </div>
        </xsl:if>
          <xsl:choose>
            <xsl:when test="($type = 'swr')">
                <label  for="q-1" class="show-for-sr"> Search within these results</label>
                <input type="search" id="q-1" name="as_q" class="q" size="{$search_box_size}" maxlength="256" value=""  autocapitalize="none" required="required"/>
                <input type="submit" class="button secondary" name="btnG" value="{$sep_bar_swr_text}" /> 
                <input type="hidden" name="q" class="q" value="{$qval}"/>
            </xsl:when>
            <xsl:when test="$show_suggest = '1' and (($type = 'home') or ($type = 'std_top'))">
                <label class="show-for-sr" for="q-1"> Search</label>
                <input type="search" id="q-1" name="q" class="q" maxlength="256" value="{$cleared_q}" autocapitalize="none" required="required"/> 
                <input type="submit" class="button secondary" name="btnG" value="{$search_button_text}" /> 
            </xsl:when>
            <xsl:otherwise>
                <label class="show-for-sr" for="q-1"> Search</label>
                <input type="search" id="q-1" name="q" class="q" maxlength="256" value="{$cleared_q}" autocapitalize="none" required="required"/>  
          ...
          <xsl:if test="$is_embedded_mode != '1'">
            <xsl:if test="($show_swr_link != '0') and ($type = 'std_bottom')">
              <a class="advanced-search-link uppercase weight-medium" ctype="advanced_swr" href="{$swr_search_url}">
                 <xsl:value-of select="$swr_search_anchor_text"/>
              </a>
            </xsl:if>
            <xsl:if test="$show_result_page_adv_link != '0'">
              <xsl:if test="($type != 'swr')">
                <a class="advanced-search-link uppercase weight-medium" ctype="advanced" href="{$adv_search_url}">
                  <xsl:value-of select="$adv_search_anchor_text"/>
                </a>
              </xsl:if>
            </xsl:if>             
  ...
  <xsl:template name="bottom_search_box">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="bottomSearch">
      <tr>
        <td bgcolor="{$sep_bar_bg_color}" align="center">
          <br/>
          <xsl:call-template name="search_box">
  ...
  Bottom search box (do not customized)
     ********************************************************************** -->
  <xsl:template name="fm_bottom_search_box">
    <xsl:call-template name="fm_search_box">
      <xsl:with-param name="type" select="'std_bottom'"/>
    </xsl:call-template>
  </xsl:template>
...
<xsl:template name="fm_gen_top_navigation">
  <xsl:if test="RES">
    <xsl:if test="$show_sort_by != '0'">
      <div class="dn-inline-block show-for-large">
        <xsl:call-template name="fm_sort_by"/>
      </div>
    </xsl:if>
  </xsl:if>
</xsl:template>
...
<xsl:template name="dynamic_navigation_results">
  ...
  <div id="main" style="{$div_pos}">
    ...
    <div id="dyn_nav">
      <!-- Onix Removed Navigate <div class="dn-hdr">
  ...
  <xsl:variable name="is_selected" select="index-of($dn_tokens, $current_token)"/>
    ...
    <xsl:choose>
      <xsl:when test="exists($is_selected)">
	    <li class="dn-attr-v selectedCat">
...
<!-- *** Result Header *** -->
 ...
 <xsl:if test="$show_res_title != '0'">
  ...
  <!-- *** Snippet Box *** -->
  <table cellpadding="0" cellspacing="0" border="0" style="table-layout: fixed; overflow-hidden; width: 100%;">
    <tr>
      <td class="s">
        ...
        <!-- <font style="font-size:{$res_url_size};  color:{$res_url_color}"> -->
          <xsl:choose>
        ...
        <xsl:if test="HN">
          <br/>
          <em>[<a ctype="sitesearch" class="f" href="{$gsa_search_root_path_prefix}?as_sitesearch={$crowded_url}&amp;{
            $search_url}">More results from <xsl:value-of select="$crowded_display_url"/></a>]</em>

        <!-- *** Link to aggregated results from database source *** -->
        <xsl:if test="starts-with($crowded_url, $db_url_protocol)">
        <em>[<a ctype="db" class="f" href="dbaggr?sitesearch={$crowded_url}&amp;{
          $search_url}&amp;filter=0">View all data</a>]</em>
          </xsl:if>
        </xsl:if>
  ...
  <xsl:text disable-output-escaping="yes">&lt;p class="hide"&gt;</xsl:text>
  </p>
...
<!-- **********************************************************************
  ...
  <xsl:template match="GM">
    <xsl:variable name="keyLink" select="GL" />
      <!--  <div class="keymatchLeft">
        <a ctype="keymatch" href="{GL}">
          <xsl:value-of select="GD"/>
        </a>
        <br/>
        <font color="{$res_url_color}">
          <span class="a">
            <xsl:choose>
              <xsl:when test="contains($keyLink, 'intcmp')">
                <xsl:value-of select="substring-before($keyLink,'?')" />
              </xsl:when>
              <xsl:otherwise>
        ...
        <div class="keymatchRight">
          <b>
            <font color="{$keymatch_text_color}">
              <xsl:value-of select="$keymatch_text"/>
            </font>
          </b>
      </div> -->
      <div class="keyMatchTable clearfix">
        <div>
          <div class="dn-inline-block text-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 39.51 39.63"><defs><style>.a,.b,.c{fill:none;}.a,.b{stroke:#777974;stroke-linejoin:round;stroke-width:1.69px;}.b{stroke-linecap:round;}</style></defs><path class="a" d="M1.5 26.96h6.76v11.83H1.5z"/><path class="b" d="M8.26 37.1H37c0-2.53-5.94-5.1-12.69-5.1H12.48"/><path class="a" d="M8.26 28.65h5.91A8.77 8.77 0 0 1 20.93 32M16.91 24.34l1.78-5.45-8.74-7.13h10.61l3.75-10.61 3.74 10.61h10.62l-8.74 7.15 3.74 11.38-9.36-6.8-3.57 2.63"/><path class="c" d="M0 0h24v24H0z"/></svg>
             <div class="reduce uppercase weight-bold">
              <xsl:value-of select="$keymatch_text"/>
            </div>
          </div>  
        </div>
      ...
      <!-- 
      <table cellpadding="0" cellspacing="0" border="0" width="100%" class="keyMatchTable">
        <tr>
          <th class="text-center" style="min-width: 118px;">
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 39.51 39.63"><defs><style>.a,.b,.c{fill:none;}.a,.b{stroke:#777974;stroke-linejoin:round;stroke-width:1.69px;}.b{stroke-linecap:round;}</style></defs><path class="a" d="M1.5 26.96h6.76v11.83H1.5z"/><path class="b" d="M8.26 37.1H37c0-2.53-5.94-5.1-12.69-5.1H12.48"/><path class="a" d="M8.26 28.65h5.91A8.77 8.77 0 0 1 20.93 32M16.91 24.34l1.78-5.45-8.74-7.13h10.61l3.75-10.61 3.74 10.61h10.62l-8.74 7.15 3.74 11.38-9.36-6.8-3.57 2.63"/><path class="c" d="M0 0h24v24H0z"/></svg>
...
<!-- **********************************************************************
  ...
  <xsl:variable name="snippet_break_start" select="'&lt;br&gt;'"/>
  <xsl:variable name="snippet_break_remove" select="''"/>
...
<!-- **********************************************************************
  ...
  <xsl:template name="reformat_keyword">
    ...
    <xsl:variable name="reformatted_3">
      <xsl:call-template name="replace_string">
        <xsl:with-param name="find" select="$snippet_break_start"/>
        <xsl:with-param name="replace" select="$snippet_break_remove"/>
 ...
 Google navigation bar in result page (do not customize)
  ...
  <xsl:template name="google_navigation">
    ...
    <xsl:variable name="fontclass">
      <xsl:choose>
        <xsl:when test="$navigation_style = 'top'"></xsl:when>
    ...
    <!--
    changes made: begin
    New item: non-breaking spacer
    Notes:
        this spacer is used to stabilize the result div in he top navigation
    -->
    ...
    <xsl:if test="($prev) or ($next)">
      ...
      <xsl:if test="$navigation_style != 'top'">
        Result Page:<xsl:call-template name="nbsp"/><xsl:call-template name="nbsp"/>
      </xsl:if>
      ...
      <xsl:choose>
        <xsl:when test="$prev">
          ...
          <xsl:if test="($navigation_style != 'google') and
        ...
        <xsl:otherwise>
          ...
          <xsl:if test="$navigation_style = 'top'">
            <span class="readOnly">Previous</span> |
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:template name="fm_bottom_sep_bar">   
  <xsl:param name="show_info"/>
  <xsl:if test="$show_info != 0">
    <xsl:if test="count(/GSP/RES/R)>0 ">
      <div class="two-column-layout search-stat-bar">
        <div class="two-column-row row">                
          <div class="column">
            ...
            <xsl:call-template name="fm_gen_top_navigation" />
          </div>
        </div>
      </div>
    </xsl:if>
 ...
 <!-- **********************************************************************
 FMTop separation bar (customize)
     ********************************************************************** -->
<xsl:template name="fm_top_sep_bar">  
    <xsl:param name="text"/>  
    <xsl:param name="show_info"/>
    <xsl:param name="time"/>  
 ...
 Top separation bar (do not customize)
  ...
  <xsl:template name="top_sep_bar">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="{$sep_bar_bg_color}" class="resultsTable">
      <tr>
        <!--<td width="1%" bgcolor="{$sep_bar_bg_color}">
          <font size="+1">
            <xsl:call-template name="nbsp"/>
            <b>
        ...
        <td align="left" bgcolor="{$sep_bar_bg_color}">
          <xsl:if test="$show_info != 0">
            <!--<font>-->
            <xsl:if test="count(/GSP/RES/R)>0 ">
              <xsl:choose>
...
<xsl:template name="copyright">
  <div>
    <br/><br/>
    <p>
    <font face="arial,sans-serif" color="#2f2f2f">
...
<!-- **********************************************************************
 Freddie Mac Templates
 ********************************************************************** -->
<xsl:template name="fm_header">
<div class="show-for-print">FreddieMac.com</div>
<a class="show-for-sr" href="#nav-end">Skip to content</a>
<header id="header-nav" class="hide-for-print">  
  <div class="top-bar ribbon show-for-xlarge data-expanded" id="ribbon">    
    <div id="ribbon-nav" class="row column">  
      <nav class="top-bar-right" role="navigation">
        <ul class="menu">
          <li id="desktop-corporate-home" class="active"><a href="http://www.freddiemac.com/">Freddie Mac Home</a></li>
          <li id="desktop-singlefamily-home"><a href="http://www.freddiemac.com/singlefamily/">Single-Family</a></li>
          <li id="desktop-multifamily-home"><a href="http://www.freddiemac.com/multifamily/">Multifamily</a></li>
          <li id="desktop-markets-home"><a href="http://www.freddiemac.com/capital-markets/">Capital Markets</a></li>
        </ul>
        <ul class="menu vertical ribbon-rbo-section" data-accordion-menu="">
          <li class="nav-accordion-parent ribbon-rbo-toggle"><a href="#">Renters, Buyers, &amp; Owners</a>
            <ul class="menu vertical nested" style="display: none;">
              <li><a href="http://myhome.freddiemac.com/"><h4>My Home by Freddie Mac<sup><xsl:call-template name="reg"/></sup></h4>
              ...
              <p>Buy a home from HomeSteps<sup><xsl:call-template name="reg"/></sup>, the real estate sales unit of Freddie Mac.</p></a>
              </li>
            </ul>
          </li> 
        </ul> 
        <ul class="menu">
          <li id="desktop-search-wrapper">              
            <div class="input-group">
              <form action="search" method="get" accept-charset="utf-8" onsubmit="return (this.q.value == '') ? false : true;">
                <div class="input-group">
    ...
    <div class="row column primarynav-buffer"> 
      <div class="top-bar"> 
        ...
        <div class="float-right">
          <button id="search-toggle" class="search-icon button" type="button" data-toggle="search-mobile" aria-label="toggle search"><svg xmlns="http://www.w3.org/2000/svg" 
		  viewBox="0 0 65.11 64.57"><defs><style>.a{fill:none;stroke-width:8px;}.a,.b{stroke:#00a6e2;}.b{fill:#00a6e2;stroke-linecap:round;stroke-width:7px;}</style></defs><circle 
		  class="a" cx="23.67" cy="23.67" r="20.17"/><line class="b" x1="38.19" y1="37.66" x2="62.11" y2="61.57"/></svg></button><button id="menu-toggle" class="toggle-icon button" 
          type="button" data-toggle="primary-nav main-nav" aria-label="toggle menu"></button>
        </div>
            <a href="http://www.freddiemac.com/" class="logo-mobile"><span>Freddie Mac Home</span><img class="no-border" src="http://www.freddiemac.com/images/FreddieMacLogo.svg" alt="Freddie Mac logo" /></a>
        </div>
        <div class="top-bar-left show-for-xlarge">
          <a href="http://www.freddiemac.com/" class="logo-desktop"><img alt="Freddie Mac logo" src="http://www.freddiemac.com/images/FreddieMacLogo.svg" class="no-border" /></a>
        </div>
        <nav class="top-bar-right" id="main-nav" role="navigation">  
          <ul class="menu vertical xlarge-horizontal">       
            <li id="nav-perspectives"><a href="http://www.freddiemac.com/perspectives/">Perspectives</a>
            </li>
            <li id="nav-research"><a href="http://www.freddiemac.com/research/">Research</a>
            </li>
            <li id="nav-blog"><a href="http://www.freddiemac.com/blog/">Blog</a>
            </li>
            <li id="nav-mediaroom"><a href="http://www.freddiemac.com/media-room/">Media Room</a>
            </li>
            <li id="nav-about"><a href="http://www.freddiemac.com/about/">About</a>
            </li>
          </ul>
          <ul class="menu vertical show-for-xlarge nav-bus-section" data-accordion-menu="">
            <li class="nav-accordion-parent nav-bus-toggle"><a href="#">Our Businesses</a>
              <ul class="menu vertical nested" style="display: none;">
                <li><a href="http://www.freddiemac.com/singlefamily/"><h4>Single-Family</h4>
                      <p>For lenders, servicers and industry partners</p></a></li>
                <li><a href="http://www.freddiemac.com/multifamily/"><h4>Multifamily</h4>
                      <p>For lenders, servicers, borrowers and investors</p></a></li>
                <li><a href="http://www.freddiemac.com/capital-markets/"><h4>Capital Markets</h4>
          ...
          <ul class="menu vertical hide-for-xlarge" data-accordion-menu="">
            <li class=""><a id="nav-mobile-bus-link" href="#">Our Businesses</a>
              <ul class="menu vertical nested" style="display: block;">
                <li><a href="http://www.freddiemac.com/singlefamily/">Single-Family</a></li>
                <li><a href="http://www.freddiemac.com/multifamily/">Multifamily</a></li>
                <li><a href="http://www.freddiemac.com/capital-markets/">Capital Markets</a></li>
              </ul>
            </li>        
            <li class=""><a id="nav-mobile-rbo-link" href="#">Renters, Buyers, &amp; Owners</a>
              <ul class="menu vertical nested" id="nav-mobile-rbo" style="display: block;">
                <li><a href="http://myhome.freddiemac.com/">My Home by Freddie Mac<sup><xsl:call-template name="reg"/></sup></a></li>
        ...
        <form action="search" method="get" accept-charset="utf-8" onsubmit="return (this.q.value == '') ? false : true;">
          <div class="row">
            ...
            <label for="mobile-search" class="show-for-sr">Site Search</label>
              <input type="search" id="mobile-search" name="q" placeholder="Site Search" required="required" maxlength="100" />	   
            </div>
        ...
        <ul class="hide no-bullet">
          <li><a id="subnav-research-toggle" class="hide-for-xlarge" data-toggle="subnav-research">Research Menu</a>
            <ul class="menu xlarge-horizontal" id="subnav-research-list">
              <li><a id="subnav-research-mortgage" href="http://www.freddiemac.com/pmms/">Mortgage Rates</a></li>
              <li><a id="subnav-research-insight" href="http://www.freddiemac.com/research/insight/">Insight</a></li>
              <li><a id="subnav-research-outlook" href="http://www.freddiemac.com/research/outlook/">Outlook</a></li>
              <li><a id="subnav-research-consumer" href="http://www.freddiemac.com/research/consumer-research/">Consumer Research</a></li>
              <li><a id="subnav-research-indices" href="http://www.freddiemac.com/research/indices/">Indices</a></li>
              <li><a id="subnav-research-datasets" href="http://www.freddiemac.com/research/datasets/">Additional Datasets</a></li>
            </ul>
          </li>
        </ul>
      </div>
        ...
        <ul class="hide no-bullet">
          <li><a id="subnav-blog-toggle" class="hide-for-xlarge" data-toggle="subnav-blog">Blog Menu</a>
            <ul class="menu xlarge-horizontal" id="subnav-blog-list">
              <li><a id="subnav-blog-homeownership" href="http://www.freddiemac.com/blog/homeownership/">Homeownership</a></li>
              <li><a id="subnav-blog-rental" href="http://www.freddiemac.com/blog/rental_housing/">Rental Housing</a></li>
              <li><a id="subnav-blog-research" href="http://www.freddiemac.com/blog/research_and_analysis/">Research &amp; Analysis</a></li>
              <li><a id="subnav-blog-notable" href="http://www.freddiemac.com/blog/notable/">Notable</a></li>
              <li><a id="subnav-blog-archive" href="http://www.freddiemac.com/blog/blog-archive/">Archive</a></li>
            </ul>
          </li>
        </ul>
      </div>
        ...
        <ul class="hide no-bullet">
          <li><a id="subnav-mediaroom-toggle" class="hide-for-xlarge" data-toggle="subnav-mediaroom">Media Room Menu</a>
            <ul class="menu xlarge-horizontal" id="subnav-mediaroom-list">
              <li><a id="subnav-mediaroom-archive" href="http://freddiemac.mwnewsroom.com/">Press Release Archive</a></li>
              <li><a id="subnav-mediaroom-resources" href="http://www.freddiemac.com/media-room/resources.html">Media Resources</a></li>
            </ul>
          </li>
        </ul>
      </div>
        ...
        <ul class="hide no-bullet">
          <li><a id="subnav-about-toggle" class="hide-for-xlarge" data-toggle="subnav-about">About Menu</a>
            <ul class="menu xlarge-horizontal" id="subnav-about-list">
              <li><a id="subnav-about-leaders" href="http://www.freddiemac.com/about/leaders/">Our Leaders</a></li>
              <li><a id="subnav-about-business" href="http://www.freddiemac.com/about/business/">Our Business</a></li>
              <li><a id="subnav-about-people" href="http://www.freddiemac.com/about/people/">Our People</a></li>
              <li><a id="subnav-about-communities" href="http://www.freddiemac.com/about/communities/">Our Communities</a></li>
              <li><a id="subnav-about-governance" href="http://www.freddiemac.com/governance/">Corporate Governance</a></li>
              <li><a id="subnav-about-investors" href="http://www.freddiemac.com/investors/">Investor Relations</a></li>
              <li><a id="subnav-about-careers" href="https://www.freddiemac.jobs/">Careers</a></li>
            </ul>
          </li>
        </ul>
      </div>
  ...
  <div class="subnav-edge" id="nav-end"></div>   
</header>    
</xsl:template>

<xsl:template name="fm_footer">
<footer class="footer hide-for-print">
	<div class="row column">
		<div class="footer-top">
        <ul class="no-bullet enlarge">
          <li><a href="https://www.freddiemac.jobs/">Careers</a></li>
          <li><a href="http://www.freddiemac.com/investors/">Investor Relations</a></li>
          <li><a href="http://www.freddiemac.com/about/people/suppliers.html">Suppliers</a></li>
          <li><a href="http://www.freddiemac.com/about/contact-us.html">Contact Us</a></li>
      ...
      <ul class="no-bullet">
        <li><a href="http://www.freddiemac.com/terms/">Terms of Use</a></li>
        <li><a href="http://www.freddiemac.com/terms/privacy.html">Privacy Policy</a></li>
        <li><xsl:call-template name="copy"/> 2017 Freddie Mac</li>
      </ul>
    </div>
	</div>
</footer>
...
<xsl:template name="fm_styles">
<meta name="theme-color" content="#88BD45" />
<link rel="shortcut icon" type="image/png" href="http://www.freddiemac.com/images/favicon.png" />
<link rel="icon" type="image/png" href="http://www.freddiemac.com/images/favicon.png" />
<link rel="stylesheet" href="http://www.freddiemac.com/ss/app_corp.css" />
<link href="https://fonts.googleapis.com/css?family=Roboto:300,400,500,700,900" rel="stylesheet" />
<style>
blockquote { 
  font-size: inherit; border-width: 0; font-weight: inherit; padding: 0; 
}
...
@media screen and (max-width: 61.24999999em) {
  .two-column-layout > .two-column-row { 
...
@media screen and (min-width: 55em) {
  ...
  .keyMatchTable > div:nth-of-type(odd) {
    width: 135px;
    display: table-cell;
    vertical-align: middle;
  } 
  .keyMatchTable > div:nth-of-type(even) {
    display: table-cell;
    vertical-align: middle;
  }
  .keyMatchTable svg {
...
<xsl:template name="fm_jsfooter">
    ...
    <script>//<![CDATA[   
    window.jQuery || document.write('<script src="http://www.freddiemac.com/js/vendor/jquery-3.2.1.min.js"><\/'+'script>')	
    //]]>
    </script>
    <script src="http://www.freddiemac.com/js/vendor/somni_code.js">
    </script>
    
    <script type="text/javascript" src="http://www.freddiemac.com/js/app_corp.js">
    </script>
    
    
  <xsl:text disable-output-escaping="yes">
&lt;script type="text/javascript"&gt;
  var num_results='</xsl:text>
    <xsl:value-of select="RES/M"/>
    <xsl:text disable-output-escaping="yes">';
&lt;/script&gt;
  </xsl:text>
...
/* You may give each page an identifying name, server, and channel on the next lines. */
  var a=QueryParam['as_q']||"",b=QueryParam['q']||"",p=QueryParam['proxycustom']||"",
   num_res=typeof(num_results)!=='undefined'?num_results:0,
   q=a!==""?decodeURIComponent(a):b!==""?decodeURIComponent(b):"";
  p=decodeURIComponent(p); 
  q=q.toLowerCase().replace(/\+inmeta:.+/,'').replace(/\+|\s+/g, " ").replace(/\s$/g, "").replace(/^\s/g, "").replace(/[^a-z0-9\300-\374.,'!():| -]/gi,"");
  s_somni.eVar1="fm|" + $('h1:first').text();
  s_somni.eVar5="fm|search";
  if(q.length>0){
	if(!p.match(/(ADVANCED|HOME)/i)){
	  s_somni.eVar4="regular|"+q;
	  s_somni.searchNull=num_res==0?true:false;    }
  }
  var widthInit = window.innerWidth || '801';
  if (parseInt(widthInit) < 800) {
    $(".g").removeClass('g').addClass('no-g');
    console.log('suppressed previews');
  }
...
;(function (g) {
	var d = document, i, am = d.createElement('script'), h = d.head || d.getElementsByTagName("head")[0],
			aex = {
				"src": d.URL.match(/freddie/)?"//gateway.answerscloud.com/freddiemac/production/gateway.min.js": "//gateway.answerscloud.com/freddiemac/staging/gateway.min.js",
				"type": "text/javascript",
				"async": "true",
				"data-vendor": "acs",
				"data-role": "gateway"
...
<!-- Start Eloqua Scripts -->

    var _elqQ = _elqQ || [];
    _elqQ.push(['elqSetSiteId', '325135053']);
...
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-W7CWC5"
...
})(window,document,'script','dataLayer','GTM-W7CWC5');
//]]>
</script>
